...
function elab(obj) {
  ...
  for (var Key in obj.elms) {
    ...
    if (show && Status == "") {
      ...
      showRow(Result, false); //chkScroll = setInterval(updateScroll, 1000);
...
function showRow(theRow, isLast) {
  fumetto += "!" + theRow;

  if (isLast) {
    righe = fumetto.substring(1, fumetto.length - 1).split(">!<");
    fumetto = "";
    fum = "";

    for (i = 0; i < righe.length; i++) {
      if (righe[i].substring(0, 1) != "<") righe[i] = "<" + righe[i];
      if (righe[i].substring(righe[i].length - 1) != ">") righe[i] = righe[i] + ">";

      if (i == 0 && righe.length > 1) {
        //togliere ultimo TAG
        righe[i] = righe[i].substring(0, righe[i].lastIndexOf("<"));
      }

      if (righe.length > 1 && i > 0 && i < righe.length - 1) {
        //togliere primo e ultimo TAG
        righe[i] = righe[i].substring(0, righe[i].lastIndexOf("<"));
        righe[i] = righe[i].substring(righe[i].indexOf(">", righe[i].lastIndexOf("avatar") + 6) + 1);
      }

      if (i - 1 == righe.length && righe.length > 1) {
        //togliere primo TAG
        righe[i] = righe[i].substring(righe[i].indexOf(">", righe[i].lastIndexOf("avatar") + 6) + 1);
      }
    ...
    chatArea.insertAdjacentHTML("beforeend", fum);
    updateScroll();
  }
}
  ...
  switch (elType) {
    ...
    case 2:
      ...
      if (Css.trim().substring(0, 6) == "button") {
        if (linkTitle != "") res += "'btn btn-primary'>" + linkTitle + "</a></div>";else res += "'btn btn-primary'>Link</a></div>";
      } else {
...
function setCookie(name, value, minutes) {
  ...
  document.cookie = name + "=" + (value || "") + expires + "; path=" + window.location.href + "; SameSite=Strict";
}
