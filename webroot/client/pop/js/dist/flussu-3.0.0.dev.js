...
var isInit = true;
var terms = "";
var scrolled = false; //var lastScrollTop = 0;
...
function startFlussu(theLang, startData, theCss) {
  ...
  if (CK != null) {
    ...
    if (flussuId != null && flussuId.length > 0 && flussuId != elms[0]) eraseCookie("flussuSid");else {
      ...
      flussuBlock = elms[2];
      flussuLang = elms[3];
      if (!startData || startData == "") terms = "R:10";else terms = startData.substring(0, startData.length - 1) + ',"R":10}';
      $("#mychat").show();
      dontstart = true;
      postParam();
    }
...
function showUserRow(text) {
  a = addUserRow(text);
  if (a) chatArea.insertAdjacentHTML("beforeend", a);
}
...
function addFlussuButton(btnData) {
  ...
  if (btnTxt != "") {
    ...
    varea = $("#flussu-btnarea"); //h = varea.height();
    //varea.height(h + 40);
...
function elab(obj) {
  ...
  if (CK == null) updateCookie(flussuId, obj.sid, obj.bid, flussuLang, 30); //setCookie("flussuSid", flussuId +","+ flussuLang, 30)
  ...
  var d = new Date();
  var btnUuid = newUuid();
  var endRow = ""; //var Ordered=obj["0RD_0"].split(",");
  //if (!Ordered)
  ...
  for (var i = 0; i < elmsOrd.length; i++) {
    ...
    switch (key) {
      ...
      case "L":
        Result += addRowChat(mVal, 1, Css); //Result += showFlussuRow(mVal);
...
function showTextArea(btn, txt, flussuBlock) {
  ...
  area.innerHTML = "";
  if (typeof txt.css == "string") css = txt.css;else css = txt.css.display_info.subtype;

  if (btn) {
    Result = " id='flussu-text-input' class='flussu-input " + flussuBlock + "' name='$" + txt.fem + "'";
    if (css == "textarea") Result = "<textarea style='height:100px' rows=3 " + Result + "></textarea><br>";else Result = "<input type='text' onkeyup='checkEnter(event,\"" + btn.uuid + "\")' " + Result + " value=''>";
    $("#flussu-btnarea").hide();
  } else {
    if (css == "textarea") Result = "<textarea style='height:100px' rows=3 id='flussu-text-input' class='flussu-input " + flussuBlock + "' name='$" + txt.fem + "'></textarea><br>";else Result = "<input id='flussu-text-input' class='flussu-input " + flussuBlock + "' type='text' name='$" + txt.fem + "' value=''>";
  }
  ...
  switch (elType) {
    ...
    case 2:
      ...
      if (typeof Css != "string" && Css.display_info.subtype == "button") {
        ...
        if (linkTitle != "") res += linkTitle + "</a></div>";else res += "Link</a></div>";
      } else {
        if (linkTitle != "") res += "'" + Css + "'>" + linkTitle + "</a></div>";else res += "'" + Css + "'>" + element + "</a></div>";
      }
...
function buildInterface() {
  ...
  if (startMsg != "") $("#flussu-startMsg").html(startMsg);else $("#flussu-startMsgElm").hide();

  _hideAll();

  var CK = getCookie("flussuSid"); //eraseCookie("flussuSid");

  if (CK != null) {
    var elms = CK.split(",");
    if (flussuId != null && flussuId.length > 0 && flussuId != elms[0]) eraseCookie("flussuSid");else {//startFlussu(null, null);
    }
  } else {
    $("#flussu-startarea").show();
  }

  getWorkflowInfo(); //send msg
