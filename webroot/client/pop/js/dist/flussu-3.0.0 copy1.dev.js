...
/************************************************************************
Flussu v2.8.0 - flussu chat interface v3.0.0
Copyright (C) 2021-2023 Mille Isole SRL - Palermo (Italy)
- scripting for whole video chat window - Jan 2023 - For Flussu v2.8
************************************************************************/
// get url parameters
...
function startFlussu(theLang, startData, theCss) {
  ...
  if (CK != null) {
    ...
    if (flussuId != null && flussuId.length > 0 && flussuId != elms[0]) eraseCookie("flussuSid");else {
      flussuId = elms[0]; //flussuSid = elms[1]
      //flussuBlock = elms[2];
...
function addRowChat(element, elType, Css) {
  switch (elType) {
    ...
    case 2:
      ...
      if (Css.display_info.subtype == "button") {
...
function setCookie(name, value, minutes) {
  ...
  document.cookie = name + "=" + (value || "") + expires + "; Path=" + flussuServer + "; SameSite=Strict";
}
